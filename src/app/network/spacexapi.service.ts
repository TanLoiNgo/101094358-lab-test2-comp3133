  ...
  getMissionsData(): Mission[] {
    this.missions$.forEach((element: any) => {
      element.forEach((eachMission: any) => {
        const { flight_number, mission_name, launch_year, details,rocket, launch_site, links } = eachMission
        const mission_patch_small: string = links.mission_patch_small
        const mission: Mission = { flight_number, mission_name, launch_year, details,rocket, launch_site, links}
